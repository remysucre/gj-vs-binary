The goal of this project is to compare [generic join](https://gitlab.com/remywang/blog/-/blob/master/posts/wcoj.md) with traditional binary join. We conjecture that the performance of any *linear* binary join plan can be matched by a plan using only generic joins. 

This of course requires benchmarking binary join and generic join side-by-side. Unlike binary join, there are virtually no high quality, open source implementation of generic join. Luckily, since relational algebra is equivalent to tensor algebra, the tensor algebra compiler [TACO](http://tensor-compiler.org) implements an algorithm for sparse tensor algebra that precisely coincides with generic join. We may therefore piggyback on TACO to answer queries. 
...
- The optimizer may occasionally choose a bushy plan, in which case we should consider how to [[combine multiple generic join plans]]. 
- Dealing with [[indices in generic join]]. 